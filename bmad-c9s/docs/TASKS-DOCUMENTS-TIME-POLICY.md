...
## ОБЯЗАТЕЛЬНО ДЛЯ ВСЕХ АГЕНТОВ (БЕЗ ИСКЛЮЧЕНИЙ)

**ПЕРЕД ЛЮБЫМ СОДЕРЖАТЕЛЬНЫМ ШАГОМ ПО PRD, ТЕХСПЕКЕ, АРХИТЕКТУРЕ, БРИФУ, ИССЛЕДОВАНИЮ, UX-ПАКЕТУ И ЛЮБОМУ АНАЛОГИЧНОМУ ДОКУМЕНТУ В `requirements/*.md` АГЕНТ ОБЯЗАН:**

1. **СНАЧАЛА** СОЗДАТЬ ФАЙЛ ЗАДАЧИ `{base}/issues/{slug}.md` С КОРРЕКТНЫМ FRONTMATTER: `created_by`, `submaster`, `creators` (все три — как `username` из `c9s-config.yaml`), `project_hash`, `hash`, `estimate`, статус работы.
2. **СРАЗУ** ВЫПОЛНИТЬ **COMMIT** И **PUSH** В РЕПОЗИТОРИЙ РЕЗУЛЬТАТОВ. БЕЗ ЭТОГО УЧЁТ ВРЕМЕНИ В CAPITAL **НЕ ЗАПУСТИТСЯ** — ОПЕРАТОР ТЕРЯЕТ ЗАФИКСИРОВАННОЕ ВОЗНАГРАЖДЕНИЕ / ВРЕМЯ.
3. **ТОЛЬКО ПОСЛЕ ПУНКТОВ 1–2** ПИСАТЬ ИЛИ РЕДАКТИРОВАТЬ `requirements/*.md` (STORY).

**НИ ОДНА КОМАНДА ИЛИ SLASH-КОМАНДА** (`/tech-spec`, `/prd`, `/bmad/…` **И Т.П.**) **НЕ ОТМЕНЯЕТ ЭТО ПРАВИЛО.** НАЗВАНИЕ КОМАНДЫ НЕ ЗАМЕНЯЕТ ЗАДАЧУ `issues/*.md`.

**КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** СОВЕТОВАТЬ «СОЗДАТЬ ЗАДАЧУ ЗАДНИМ ЧИСЛОМ», «ДОБАВИТЬ ISSUE ПОСЛЕ ФАКТА» ИЛИ ИНОЙ ОБХОД ОЧЕРЁДНОСТИ УЧЁТА. ЭТО НЕСОВМЕСТИМО С ПРОЦЕССОМ CAPITAL И ВВОДИТ ОПЕРАТОРА В ЗАБЛУЖДЕНИЕ.

**ЕДИНСТВЕННОЕ ИСКЛЮЧЕНИЕ:** ПОЛЬЗОВАТЕЛЬ **ЯВНО И ДОСЛОВНО** НАПИСАЛ, ЧТО **КОНКРЕТНО ЭТОТ** ШАГ **ВНЕ** УЧЁТА CAPITAL ИЛИ **МОЖНО БЕЗ ЗАДАЧИ `issues/*.md`**. БЕЗ ТАКОЙ ЯВНОЙ ФРАЗЫ У АГЕНТА **НЕТ** ПРАВА НА ИСКЛЮЧЕНИЕ.

---

**Кратко (для человека):** Крупные шаги (PRD, архитектура, бриф, UX-пакет…) — **сначала** задача **`issues/*.md`** + **push**, **затем** содержание в **`requirements/*.md`**. У новой задачи в frontmatter — **`created_by`**, **`submaster`**, **`creators`** = твой **`username`** из `c9s-config.yaml`; старт работы — **commit + push**. По умолчанию **`estimate: 0`**. В **`on_review`** задачу переводит **только** оператор явной командой (**`/c9s-submit-for-review`** и т.п.) — агент сам так не делает. Ниже — полный текст правил.
...
Исключения — см. блок **«ОБЯЗАТЕЛЬНО ДЛЯ ВСЕХ АГЕНТОВ»** в начале документа: у агента **нет** самодельных исключений; только явная фраза пользователя про шаг вне учёта или «можно без задачи».
