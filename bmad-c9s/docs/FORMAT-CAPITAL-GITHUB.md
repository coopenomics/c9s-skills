...
- **ОБЯЗАТЕЛЬНО ДЛЯ АГЕНТОВ (БЕЗ ИСКЛЮЧЕНИЙ):** порядок **СНАЧАЛА** `issues/*.md` + **commit + push**, **ЗАТЕМ** `requirements/*.md` — см. блок заглавными буквами в начале [TASKS-DOCUMENTS-TIME-POLICY.md](TASKS-DOCUMENTS-TIME-POLICY.md). Команды вроде `/tech-spec` **не** отменяют это.
