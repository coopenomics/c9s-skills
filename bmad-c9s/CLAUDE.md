...
## ОБЯЗАТЕЛЬНО ДЛЯ АГЕНТОВ (CAPITAL / УЧЁТ ВРЕМЕНИ, БЕЗ ИСКЛЮЧЕНИЙ)

**СНАЧАЛА `issues/*.md` + COMMIT + PUSH, ПОТОМ `requirements/*.md`.** Любой PRD, техспека, архитектура, бриф, UX и т.д. — **ТОЛЬКО** в таком порядке. Команды вида `/tech-spec`, `/prd` **НЕ** освобождают от задачи.

**ЗАПРЕЩЕНО** предлагать оформлять задачу «задним числом» или после факта работы — это ломает учёт и недопустимо.

Полный текст: [TASKS-DOCUMENTS-TIME-POLICY.md](docs/TASKS-DOCUMENTS-TIME-POLICY.md).
